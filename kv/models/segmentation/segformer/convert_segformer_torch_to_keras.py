...
keras_model: keras.Model = SegFormerB0(weights=None, num_classes=150, input_shape=(512,512,3))
temp_path = download_weights(torch_model_path_mit_b0)
torch_model: torch.nn.Module = torch.load(temp_path, map_location="cpu")

trainable_keras_weights, non_trainable_keras_weights = split_model_weights(keras_model.backbone)
...
for keras_weight, keras_weight_name in tqdm(
    ...
    if "attention" in torch_weight_name:
        transfer_attention_weights(keras_weight_name, keras_weight, torch_weights_dict['state_dict'], name_replacements={"block":"backbone.block"})
        continue
...
bn_layer.weights[0].assign(pytorch_state_dict['decode_head.linear_fuse.bn.weight'].cpu().numpy())
bn_layer.weights[1].assign(pytorch_state_dict['decode_head.linear_fuse.bn.bias'].cpu().numpy())
bn_layer.weights[2].assign(pytorch_state_dict['decode_head.linear_fuse.bn.running_mean'].cpu().numpy())
bn_layer.weights[3].assign(pytorch_state_dict['decode_head.linear_fuse.bn.running_var'].cpu().numpy())
...
keras_model.get_layer('SegFormer_B0_head_classifier').weights[0].assign(final_conv_weight)
...
# Save the model
keras_model.save('SegFormerB0_ADE.keras')