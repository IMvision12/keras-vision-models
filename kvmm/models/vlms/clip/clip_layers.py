...
class CLIPAttention(keras.layers.Layer):
    ...
    """
    def __init__(self, proj_dim, num_heads, num_hidden_layers, name_prefix=None, **kwargs):
        ...
        out_proj_name = f"{self.name_prefix}_out_proj" if self.name_prefix else "out_proj"
        ...
        self.out_proj = keras.layers.Dense(self.proj_dim, use_bias=True, name=out_proj_name)
...
class CLIPLogitScale(keras.layers.Layer):
    ...
    def build(self, input_shape):
        if not isinstance(input_shape, list) and len(input_shape) != 2:
            raise ValueError("CLIPLogitScale expects a list of 2 input shapes (image_embeddings, text_embeddings)")
    
        self.logit_scale = self.add_weight(
            shape=(),
            initializer=keras.initializers.Constant(value=ops.log(1 / self.initial_value)),
            trainable=True,
            name="logit_scale",
        )
