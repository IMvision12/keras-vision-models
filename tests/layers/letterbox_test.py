...
class TestLetterbox(TestCase):
    def setUp(self):
        ...
        self.test_inputs_batch = keras.random.uniform(
            self.input_shape, minval=0.0, maxval=1.0
        )
        self.test_inputs_single = keras.random.uniform(
            self.single_image_shape, minval=0.0, maxval=1.0
        )
    ...
    def test_numpy_input_compatibility(self):
        numpy_input = np.random.uniform(0.0, 1.0, self.single_image_shape).astype(
            np.float32
        )
