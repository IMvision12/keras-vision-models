...
class TestCaiT(BackboneTestCase):
    ...
    def test_backbone_features(self):
        ...
        for i, feature_map in enumerate(features):
            ...
            self.assertIn(len(feature_map.shape), (3, 4),
                f"Feature map {i} should be a 3D (transformer) or 4D (CNN) tensor, "
                f"got shape {feature_map.shape}"
            )

            self.assertEqual(feature_map.shape[0], self.batch_size,
            ...
            if is_transformer_output:
                ...
                self.assertTrue(seq_len > 0 and channels > 0,
                ...
                if i > 0:
                    ...
                    self.assertLessEqual(seq_len, prev_seq_len,
