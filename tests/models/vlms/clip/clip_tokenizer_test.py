    ...
    def test_expected_tokenization_values(self):
        ...
        for case in test_cases:
            with self.subTest(text=case["text"], description=case["description"]):
                ...
                eos_positions = [i for i, token_id in enumerate(input_ids) if token_id == self.tokenizer.eos_token_id]
                if eos_positions:
                    actual_tokens = input_ids[:eos_positions[0] + 1].tolist()
                else:
                    actual_tokens = [token_id for token_id in input_ids if token_id != self.tokenizer.eos_token_id or token_id == self.tokenizer.eos_token_id]
                    actual_tokens = actual_tokens[:len(case["expected_tokens"])]
                
                self.assertListEqual(actual_tokens, case["expected_tokens"],
