...
from kvmm.models.vlms.siglip.siglip_processor import SigLIPProcessor
...
class TestSigLIPProcessor(TestCase):
    ...
    def test_processor_initialization_default(self):
        ...
        self.assertEqual(processor.vocab_size, processor.tokenizer.vocab_size)
        self.assertIsNotNone(processor.pad_token_id)
        self.assertIsNotNone(processor.eos_token_id)
        self.assertIsNotNone(processor.unk_token_id)
    ...
    def test_processor_initialization_custom(self):
        ...
        self.assertEqual(config["image_resolution"], 384)
    ...
    def test_custom_image_resolution(self):
        processor = SigLIPProcessor(image_resolution=336)
    ...
    def test_custom_context_length(self):
        processor = SigLIPProcessor(context_length=128)
    ...
    def test_image_resize_and_crop_options(self):
        processor_resize = SigLIPProcessor(do_resize=True, do_center_crop=True)
        ...
        processor_no_resize = SigLIPProcessor(do_resize=False, do_center_crop=False)
    ...
    def test_serialization(self):
        ...
        config = processor.get_config()
        recreated_processor = SigLIPProcessor.from_config(config)
    ...
    def test_multiple_configuration_combinations(self):
        ...
        for config in configs:
            with self.subTest(config=config):
                processor = SigLIPProcessor(**config)
                result = processor(
                    text=self.sample_text_single, images=self.sample_image_array
                )
